...
## DEC-091 — The cache is keyed by source, and the label follows the bytes

*Settled · 09-15 · D16*

**Decision**

**Every cached row carries the source that wrote it, that source is part of
the table's primary key, and a read states the source it will accept.** A row
written in the other mode is a **miss**, not a stale hit. A response's
`source` field is `fetcher.get`'s `meta['source']` — the provenance of the
bytes being returned — and never `DATA_SOURCE`. Two routes still report the
configured mode, `/health` because it is a configuration readout and
`/symbols` because that list genuinely is read from whichever place the mode
selects; both say so in a comment.

**Why**

Minutes after the live key was switched on, `/quote?symbol=SPY` returned
733.4011 with a `latest_trading_day` of 2026-08-26 and `stale: false`, in an
envelope reading `"source": "live"`. Every field was behaving as written. The
price was generated by the fixture layer during the T26b rehearsal eighteen
days earlier; `quotes` was keyed on `symbol` alone so it read back as a hit
inside TTL; and the envelope reported the mode the service was configured in,
which is a fact about the config file rather than about the number beside it.

This is the worst class of bug the project can have. The page's entire claim
on a reader is that it says where a figure came from, and there is no reading
of the screen that catches this one — the number is plausible, the label is
confident, and the two are unrelated.

**Three things fell out of building it that were not in the defect.**

1. **The bars needed the column too, not just the series row.** `daily_bars`
   is keyed on symbol and date and a refresh only replaces the dates it
   returns, so tagging `daily_series` alone would hand back a live series row
   stapled to every fixture bar outside its window — one series assembled
   from two markets, invisible on screen.
2. **The source belongs in the primary key, not merely in the row.** Keyed on
   symbol alone the two modes evict each other, so an afternoon of debugging
   in fixture mode would leave the live cache cold — about fifteen of the
   day's twenty-two calls to refill (D17). Two sources is the whole
   cardinality; it cannot grow past double.
3. **A disagreement resolves to `fixture`, and there is no third label.** The
   sector grid combines eleven answers, and the obvious envelope value for a
   mixed one is `mixed` — which every view on the page would read as
   not-`fixture` and quietly take the sample-data banner down. That is this
   same defect one level up. If any row is invented, the envelope says
   invented.

**On `stale`.** The defect asked whether a fresh row whose source does not
match should ever be reported unstale. The question dissolves: such a row is
never served at all, so there is nothing to label either way. A mislabelled
fresh row cannot arise rather than being reported carefully.

**Migration.** A database written before this is dropped and rebuilt — the
source cannot be added to a primary key by `ALTER TABLE`, and every row in it
is a cached payload whose provenance is exactly what is unknown.
`upstream_calls` is deliberately **not** dropped: it is the ledger the daily
