...
- [ ] **O6 · Audit a feature.** Take the least-recently-audited part of the page
  ...
  of the four verdicts and record it as a row in `docs/AUDIT-LOG.md`. If the verdict is
...
**Moved to `docs/AUDIT-LOG.md` on 09-26.** Still read in full every session at
step 2 — it is how a session knows which surface is due. It left because it is
the one section here that grows forever by design (`DEC-069`: `O6` never
completes), and a permanent record sharing a ceiling with a live queue charged
four consecutive sessions rent on entries they had just written (`N18`). The
rules that govern a row, the queue's state and the log itself are in that file.
`## Done` stays here — `DEC-068`, and closed tasks do not grow without bound.
