...
rather than the 544px desktop gives it. If that width ever stops working, this is
the only picture that would show it. (`t10-sectors` used to carry that shot and
was superseded here: `t11-fundamentals` photographs the same grid in the same
window, with the rest of the page as it now stands.)

Nothing else needs one. The watchlist's one width-dependent behaviour is the
trend column, present at 768 and 1440 and hidden below 460, so desktop and mobile
bracket it; the chart and the filings panel look at 768 exactly as they do at
1440.

## The `t8-*` and `t9-*` sets predate the filings panel

They are kept for the states they hold and nothing else — a chart range, a symbol
that does not exist, a blocked `localStorage`. Below the fold each of them shows a
quote card that has since lost two figures and a page that has since grown a
surface. Read them for their state, not for the layout; `t11-*` is what the page
looks like.
