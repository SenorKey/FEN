...
## 2026-09-15 — D16, D17 and S6: the cache stopped lying, and calls got spaced
**Outcome:** shipped — both open in-bounds defects closed, memory consolidated
**Changed:** `server/store.py`, `server/fetcher.py`, `server/collect.py`,
`server/incisor.py`, four service test modules, `BACKLOG.md`, `DECISIONS.md`,
`DECISIONS-DETAIL.md`
**Verified:** 244 service tests and 245 page tests green; both incident
scenarios replayed against the fixed code and behaving as claimed
**Notes:** three tasks, the session cap.

**Step 1 did not stop this session, and that was the call to get right.** The
tree held `?? doe-v-bonnell/.claude/` — the same `launch.json` and `serve.py`,
still untouched since 09-09. `DEC-084` settled on 09-11 that an untracked file
does not fail the clean-tree check and a changed *tracked* one does, precisely
so a stray preview config cannot skip every day. 09-12 met the same files and
skipped anyway. Three days had gone to two files nobody has edited in a week,
so this session read the entry it was given and worked, in its own worktree
(`DEC-081`) with Key's checkout untouched throughout. `DEC-084`'s detail now
records that it was contradicted once and held.

### D16 — a price that could not say where it came from

The worst defect this project has had, and the one with no visible symptom.
`quotes` was keyed on symbol alone, so after the live key went on, SPY came
back at **733.4011** — generated by the fixture layer during the T26b
rehearsal eighteen days earlier — inside TTL, `stale: false`, in an envelope
reading `"source": "live"`. Every field was behaving exactly as written. The
`source` field reported the *configured mode*, which is a fact about
`config.env` and not about the number beside it.

Every cached row now records what wrote it, that source is part of the primary
key, and a read states the source it accepts: a row from the other mode is a
**miss**, not a stale hit. Responses report `meta['source']` — the provenance
of the bytes. `/health` and `/symbols` still report the configured mode and
each now says why in a comment.

Three things fell out of building it that the defect did not name:

1. **The bars needed the column too.** `daily_bars` is keyed on symbol and
   date and a refresh only replaces the dates it returns, so tagging the
   series row alone would hand back a live series stapled to every fixture bar
   outside its window — one series from two markets, invisible on screen.
2. **The source belongs in the key.** Keyed on symbol alone the two modes
   evict each other, so an afternoon debugging in fixture mode would leave the
   live cache cold — about fifteen of the day's calls to refill.
3. **A disagreement resolves to `fixture`, with no third label.** The sector
   grid combines eleven answers and `mixed` was the obvious envelope value for
   a mixed one. Every view on the page asks `source === 'fixture'` to decide
   whether to show the sample-data banner, so a word none of them know would
   have read as live and taken the banner down. That is this same defect one
   level up, and it was two lines from shipping.

Replayed through the route: fixture mode serves 733.4011 labelled `fixture`;
flipped to live with upstream unreachable it answers **503 `data_unavailable`**
rather than relabelling; flipped back, the fixture cache is still there.

### D17 — the limit with a number and no enforcement

The daily budget had a table, a counter, a reserve and a design. The
per-minute limit had nothing, so four `/history` calls left in eight
milliseconds, drew the throttle, and spent four calls on replies carrying no
data. Calls are now spaced twelve seconds — the documented five a minute.

**Spacing rather than a window, and declining rather than sleeping.** A
five-per-sixty window would have *permitted* that burst, four being fewer than
five, and it was throttled anyway — so the burst is what upstream objects to.
And the service runs a single worker, so waiting for a slot means twelve
seconds in which the whole page is unanswerable, three minutes of it on a cold
cache. A refusal instead lands in the path an exhausted budget already uses,
so the dashboard fills over the next few page loads rather than blocking on
one. Replayed: the four-symbol burst now makes **one** upstream call and
declines three before they leave the box.

**The sector grid keeps its eleven funds**, which D17 asked to decide. At a
week's TTL they amortise to ~1.6 calls a day; the lapse day is now survivable
rather than self-inflicted. A knock-on worth knowing:
`SECTOR_REFRESH_PER_REQUEST` is 2 and no longer binds in live mode, because
the pacer is tighter. Kept as a ceiling, with `test_sectors.py` asserting both
— if that test ever reads two again, pacing is not running.

### S6 — because the next session could not have filed anything

Filing D16 and D17 took `DECISIONS.md` to 14,250 of 14,500: one entry of room,
which guide §16 calls a deadlock rather than a budget. Three lossless cuts,
landing at **13,925 — 575 free, four or five entries**:

- `DEC-089` was a second copy of a comment already in `js/chart-geometry.js`,
  the same file `DEC-018` left for on 09-12. Moved out, index row and detail
  entry both.
- `DEC-028`, `DEC-086` and `DEC-088` stated one rule three times — the third
  announced itself as "third cap of the same kind". Merged into one row naming
  all three numbers; `DEC-086` keeps its separate cash-holdback claim.
- The preamble kept its own copy of `DEC-087`'s moved-out list.

**The ratchet stays at 14,500.** A 325-byte gain does not earn a lower
ceiling, and moving it would take back the room this pass just made. The
honest finding is that this file is near its floor: 09-12 did the deep pass
three days ago, and what is left is dense. The next consolidation will have to
move rows into the files they bind rather than trim, and the two obvious
candidates are not available — `DEC-047` carries D7's three rejected
approaches and `DEC-048`'s own entry argues it must stay in shared memory.

While there: the destinations of six rows moved out before this pass existed
only in `PROGRESS.md` entries the tail no longer reaches, which is the failure
§16 names. They are now in `DEC-087`'s detail entry. Two of the six could not
be attributed from the journal and say so rather than being guessed at.

Nothing to prune in `docs/shots/` — six committed images, both look sets live:
broadsheet pending T13c, workbench the recorded comparison.

### For Key

**Three surfaces are now due an audit and none was taken.** T14 and T15
shipped 09-11, T16 on 09-12, and every one is well past the three-session
mark. Guide §14 ranks a due audit above a backlog task, and §18 makes it a
whole session's work judged from `shoot.py` images — which is not something to
bolt onto the end of a session that had already closed two defects. **The next
session should take T14's audit and nothing else**, before T13c or T17.

**N14 · still open, unchanged**, and now with a data point: the wording
question cost 09-12 a day. `DEC-084` holds and is what this session followed.
